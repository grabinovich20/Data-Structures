...
TEST(Dynamic, SixElements) {
    stack stk;
    vector<string> strings = {"alpha", "beta", "gamma", "delta", "epsilon", "zeta"};
    ASSERT_TRUE(stk.is_empty());
    for(size_t i = 0; i < strings.size(); i++) {
        stk.push(strings[i]);
        ASSERT_EQ(stk.size(), i + 1);
...
TEST(Dynamic, TenElements) {
    stack stk;
    vector<string> strings = {"alpha", "beta", "gamma", "delta", "epsilon", "zeta", "eta", "theta", "iota", "kapppa"};
    ASSERT_TRUE(stk.is_empty());
    for(size_t i = 0; i < strings.size(); i++) {
        stk.push(strings[i]);
        ASSERT_EQ(stk.size(), i + 1);
...
TEST(Dynamic, UsesDynamicAllocation) {
    stack stk;
